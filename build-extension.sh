...
cp manifest.json extension-background.js index.html popup.html popup.js picker.js styles.css ios-polish.css app.js ui-polish.js adsense.js adsense-config.js monetization.js chrometry-icon.svg dist/chrometry-extension/
...
API_BASE="$API_BASE" WEB_URL="$WEB_URL" python3 - <<'PY'
...
api_url = os.environ.get("API_BASE", "").strip()
if api_url:
...
if './chrometry-config.js' not in s:
...
(
  ...
  cd chrometry-extension
  zip -qr ../chrometry-extension.zip .
)

...
function verifySignedToken(token){if(!token||typeof token!=='string'||!token.includes('.'))throw new Error('Missing Pro license.');const [body,sig]=token.split('.');const secret=process.env.CHROMETRY_LICENSE_SECRET;if(!secret)throw new Error('CHROMETRY_LICENSE_SECRET is not configured.');const expected=crypto.createHmac('sha256',secret).update(body).digest('base64url');if(sig.length!==expected.length||!crypto.timingSafeEqual(Buffer.from(sig),Buffer.from(expected)))throw new Error('Invalid Pro license.');const p=JSON.parse(Buffer.from(body,'base64url').toString('utf8'));if(!p.sub||!p.customer||!p.exp||p.exp<Math.floor(Date.now()/1000))throw new Error('Expired Pro license.');return p;}
function bearer(req){const h=req.headers.authorization||'';return h.startsWith('Bearer ')?h.slice(7):'';}
async function activeSubscription(token){const payload=verifySignedToken(token);const subscription=await stripe().subscriptions.retrieve(payload.sub);const active=['active','trialing'].includes(subscription.status);return {payload,subscription,active};}
